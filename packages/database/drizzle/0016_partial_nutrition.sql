...
WITH evidence AS (
  SELECT "snapshot"."id", nutrient.*
  FROM "calculation_snapshot" AS "snapshot"
  CROSS JOIN LATERAL (
    SELECT
    ...
    FROM jsonb_array_elements("snapshot"."input_snapshot"->'mealItems') AS item
  ) AS nutrient
)
UPDATE "calculation_snapshot" AS "snapshot"
SET "nutrient_evidence" = jsonb_build_object(
  'energyMillicalories', jsonb_build_object('value', "snapshot"."energy_millicalories", 'knownValue', evidence.energy_known, 'missingItemCount', evidence.energy_missing, 'completeness', CASE WHEN evidence.energy_missing = 0 THEN 'complete' ELSE 'partial' END),
  'carbohydrateMg', jsonb_build_object('value', "snapshot"."carbohydrate_mg", 'knownValue', evidence.carbohydrate_known, 'missingItemCount', evidence.carbohydrate_missing, 'completeness', CASE WHEN evidence.carbohydrate_missing = 0 THEN 'complete' ELSE 'partial' END),
  'proteinMg', jsonb_build_object('value', "snapshot"."protein_mg", 'knownValue', evidence.protein_known, 'missingItemCount', evidence.protein_missing, 'completeness', CASE WHEN evidence.protein_missing = 0 THEN 'complete' ELSE 'partial' END),
  'fatMg', jsonb_build_object('value', "snapshot"."fat_mg", 'knownValue', evidence.fat_known, 'missingItemCount', evidence.fat_missing, 'completeness', CASE WHEN evidence.fat_missing = 0 THEN 'complete' ELSE 'partial' END),
  'fiberMg', jsonb_build_object('value', "snapshot"."fiber_mg", 'knownValue', evidence.fiber_known, 'missingItemCount', evidence.fiber_missing, 'completeness', CASE WHEN evidence.fiber_missing = 0 THEN 'complete' ELSE 'partial' END)
)
FROM evidence
WHERE "snapshot"."id" = evidence."id";--> statement-breakpoint
